...
def calculate_gender_category_revenue_last_year(weekly_ranges, year_label="Last Year"):
    """Calculates weekly Gross Revenue grouped by Gender and Category for last year's week ranges."""
    ...
    for week in weekly_ranges:
        ...
        # ✅ Log revenue values
        print("\n📊 **Revenue Breakdown by Gender & Category (Last Year):**")
...
# ✅ Get last year's last 8 weeks dynamically
last_8_weeks_last_year, _ = get_last_8_weeks_last_year()

# ✅ Run calculations for Last Year
gender_category_revenue_last_year = calculate_gender_category_revenue_last_year(last_8_weeks_last_year)

if gender_category_revenue_last_year.empty:
    print("\n❌ No revenue data found for Gender & Category (Last Year). Exiting...")
    sys.exit(1)

# ✅ Sort DataFrame
gender_category_revenue_last_year["Year Type"] = pd.Categorical(
    gender_category_revenue_last_year["Year Type"], categories=["Last Year"], ordered=True
)
gender_category_revenue_last_year = gender_category_revenue_last_year.sort_values(by=["Gender", "Product Category", "ISO Week"], ascending=[True, True, True])
...
output_path = os.path.join(OUTPUT_DIR, "gender_category_ly_raw.csv")
gender_category_revenue_last_year.to_csv(output_path, index=False)

# ✅ Print confirmation with file path and preview of saved data
print(f"\n✅ Successfully saved Last Year's Gender & Category Revenue data to: {output_path}")
print("\n📂 **Saved CSV Preview (First 10 Rows):**")
print(gender_category_revenue_last_year.head(25).to_string(index=False))
