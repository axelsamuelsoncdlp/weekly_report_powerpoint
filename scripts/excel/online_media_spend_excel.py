...
def update_online_media_spend_excel():
    """Updates the Excel sheet 'online_media_spend' with finalized spend data in thousands and opens it."""

    BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
    EXCEL_FILE = os.path.join(BASE_DIR, "data", "weekly_report.xlsm")
    CSV_FILE = os.path.join(BASE_DIR, "data", "final", "online_media_spend_final.csv")
    ...
    for row_idx, row_vals in enumerate(df_parsed.itertuples(index=False), start=start_row):
        ...
        for col_idx, val in enumerate(row_vals, start=start_col):
            ...
            if isinstance(val, (int, float)):
                if pd.notna(val):
                    cell.value = val  # Values are already in thousands (e.g., 57.2)
                    cell.number_format = "#,##0.0"  # ✅ Display as thousands with 1 decimal: 57.2
                    print(f" → Converted to spend thousands: {cell.value}k")
                else:
                    cell.value = None
    ...
    print("\n✅ Excel file saved and updated with **ALL** online media spend data (in thousands)!")
    print("💡 **Note:** Values are displayed in thousands (e.g., 57.2 = $57,200)")
...
if __name__ == "__main__":
    update_online_media_spend_excel()
