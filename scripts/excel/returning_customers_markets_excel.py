...
def update_returning_customers_markets_excel():
    """Updates the Excel sheet 'returning_customers_markets' with finalized returning customers data and opens it."""

    BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
    EXCEL_FILE = os.path.join(BASE_DIR, "data", "weekly_report.xlsm")
    CSV_FILE = os.path.join(BASE_DIR, "data", "final", "returning_customers_markets_final.csv")
    ...
    ws = wb["returning_customers_markets"]
    ...
    for row_idx, row_vals in enumerate(df_parsed.itertuples(index=False), start=start_row):
        ...
        for col_idx, val in enumerate(row_vals, start=start_col):
            ...
            if isinstance(val, (int, float)):
                if pd.notna(val):
                    cell.value = int(val)  # ✅ Convert to integer for customer count
                    cell.number_format = "0"  # ✅ Apply number formatting: whole number for customer count
                    print(f" → Converted to int: {cell.value}")
                else:
                    cell.value = None
    ...
    print("\n✅ Excel file saved and updated with **ALL** returning customers markets data!")
...
if __name__ == "__main__":
    update_returning_customers_markets_excel()
