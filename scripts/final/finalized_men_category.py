...
INPUT_FILE = os.path.join(BASE_DIR, "data", "raw", "men_category_revenue_raw.csv")
CSV_OUTPUT_FILE = os.path.join(BASE_DIR, "data", "final", "men_category_revenue_final.csv")
...
def sort_and_save_men_category_data(input_file, csv_output):
    """Loads Men Category Revenue data, merges categories, sorts it, and saves as CSV."""
    ...
    required_columns = {"Calendar Year", "ISO Week", "Product Category", "Value", "Year Type", "Gender"}
    ...
    df_sorted = df.sort_values(by=["Gender", "Product Category", "Year Type", "SortOrder"]).drop(columns=["SortOrder"])

    print("\n📊 **Step 4: Sorted Men Category Revenue Data Before Pivoting**")
    ...
    print("\n📊 **Step 5: Men Category Revenue Data After Pivoting**")
    print(df_pivot.head(20))

    # ✅ Save the sorted and formatted data as a CSV file
    df_pivot.to_csv(csv_output, index=False)

    print(f"\n✅ Successfully saved formatted Men Category Revenue data to CSV: {csv_output}")


# ✅ Run the script
if __name__ == "__main__":
    sort_and_save_men_category_data(INPUT_FILE, CSV_OUTPUT_FILE)